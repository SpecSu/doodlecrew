...
const DrawingTool: React.FC<DrawingToolProps> = ({
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    setIsDrawing(true);
    setCurrentPath([{ x: coords.x, y: coords.y }]); // 开始一条新路径
  };
  
  // 鼠标移动事件
  const handleMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    if (!isDrawing || isTouchDrawing) return;
    
    const coords = getCoordinates(e.clientX, e.clientY);
    if (!coords) return;
    
    setCurrentPath(prevPath => [...prevPath, { x: coords.x, y: coords.y }]);
  };
  
  // 鼠标抬起事件
    const handleMouseUp = () => {
      if (isTouchDrawing) return; // 如果正在进行触摸绘制，忽略鼠标事件
  ...
  return (
    <div className="drawing-tool">
      ...
      <canvas
        ...
        style={{ touchAction: 'none' }} // 防止触摸设备上的默认行为
      />

...
function App() {
  ...
  // 导出鱼数据
  const handleExportFish = () => {
    const dataStr = JSON.stringify(fish);
    const dataUri = 'data:application/json;charset=utf-8,'+ encodeURIComponent(dataStr);
    
    const exportFileDefaultName = `fish-data-${new Date().toISOString().split('T')[0]}.json`;
  ...
  // 导入鱼数据
  const handleImportFish = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const importedFish = JSON.parse(content);
        
        if (Array.isArray(importedFish)) {
          // 验证导入的数据结构
          if (importedFish.every(f => f.id && f.pathSegments)) {
            setFish(importedFish);
            alert('鱼数据导入成功！');
          } else {
            alert('导入的数据格式不正确，请检查文件内容。');
          }
        } else {
          alert('导入的数据必须是数组格式。');
        }
